      ...
      <table className="w-full text-sm table-fixed">
        <thead className="sticky top-0 z-10 bg-navy-900">
          <tr className="text-xs uppercase tracking-wider text-slate-400 border-b border-slate-700">
            <th className="w-[24%] px-5 py-4 text-left font-semibold">Gerencia Corporativa / Área</th>
            <th className="w-[20%] px-5 py-4 text-left font-semibold max-sm:hidden">Empresa</th>
            <th className="w-[14%] px-5 py-4 text-center font-semibold">Cantidad</th>
            <th className="w-[7%] px-5 py-4 text-center font-semibold max-md:hidden">País</th>
            <th className="w-[20%] px-5 py-4 text-right font-semibold">Costo</th>
            <th className="w-[15%] px-5 py-4 text-right font-semibold">Editar</th>
          </tr>
        </thead>
          ...
          {gerencias.map((g) => {
            ...
            return (
              <Fragment key={g.idKey}>
                ...
                {expandido && obtenerDatosAgrupadosAreas(data, cacheEdiciones, paisActual, g.idKey).map((a) => (
