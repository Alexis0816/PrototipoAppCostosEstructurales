...
export function calcAgregado(personas, periodo) {
  let sueldoMensual = 0, bonoTargetAnual = 0, medicinaPrepagadaAnio = 0;
  personas.forEach((p) => {
    sueldoMensual += p.sueldoMensual;
    bonoTargetAnual += p.bonoTargetAnual;
    medicinaPrepagadaAnio += p.medicinaPrepagadaAnio;
  });
  return calc({ sueldoMensual, bonoTargetAnual, medicinaPrepagadaAnio, tipoSalario: 'F' }, periodo);
}

export function consolidar(coleccion, cacheEdiciones) {
  const resueltos = coleccion.map((colab) => resolverColaborador(colab, cacheEdiciones));
  let totalSueldo = 0, totalBono = 0, totalMedicina = 0;
  resueltos.forEach((c) => {
    totalSueldo += c.sueldoMensual;
    totalBono += c.bonoTargetAnual;
    totalMedicina += c.medicinaPrepagadaAnio;
  });
  return { totalSueldo, totalBono, totalMedicina, resueltos };
}
