-- ============================================================================
-- 15_update_sp_CalcularCostos.sql  |  PeopleAnalytics
-- Actualiza sp_CalcularCostos para calcular los Ingresos por país (nuevos,
-- 2026-08-19) y persistirlos en Resultados_Calculo vía MERGE.
--
-- Fórmulas nuevas (verificadas contra VS Code y el doc de migración):
...
--   CO_IngresoAnual   = ingresoMensual * 12 + bonoCPTarget
-- El ×14 (12 meses + 2 gratificaciones) aplica sobre el INGRESO MENSUAL completo
-- (sueldo + vales + comisiones), no solo sobre el sueldo base.
-- Utilidades se lee de ColaboradoresCostos (editable, escrito a mano); no se
-- duplica en Resultados_Calculo: la vista la expone directo desde la tabla base.
...
BEGIN
    ...
    DECLARE
        ...
        -- Fila contadora para el PRINT final (evita error 4083 de Azure SQL:
        -- @@ROWCOUNT no es fiable si la conexión se recupera a mitad del batch).
        @FilasProcesadas INT = 0;
    ...
    Base AS (
        SELECT
            ...
            ISNULL(c.Utilidades,           0) AS Utilidades,  -- PE/EC [SETEADO, escrito a mano]
    ...
    CalcCOFinal AS (
        SELECT co.*,
            ...
            -- Nuevos (2026-08-19): ingreso mensual = sueldo; anual = ingresoMensual*12 + bono target
            co.SM            AS CO_IngresoMensual,
            co.CO_SalAnual + co.CO_BonoTarget  AS CO_IngresoAnual
        FROM CalcCO2 co
    ),
    ...
    CalcPE AS (
        SELECT
            b.NumeroID,
            b.SM, b.Vales, b.Coms, b.AsigFam, b.Utilidades AS PE_Utilidades,
    ...
    CalcPEAnual AS (
        SELECT pf.*,
            ...
            -- Nuevos (2026-08-19): ingreso = sueldo + vales + comisiones; anual = ingresoMensual*14 + bono + utilidades
            pf.SM + pf.Vales + pf.Coms          AS PE_IngresoMensual,
            (pf.SM + pf.Vales + pf.Coms) * 14.0
            + pf.PE_BonoCPTarget + pf.PE_Utilidades  AS PE_IngresoAnual
        FROM CalcPEFinal pf
    ),
    ...
    CalcEC AS (
        SELECT
            b.NumeroID,
            b.SM, b.Coms AS EC_Coms, b.Seguro, b.Utilidades AS EC_Utilidades,
    ...
    CalcECCarga AS (
        SELECT ea.*,
            ...
            -- Nuevos (2026-08-19): ingreso = sueldo + comisiones; anual = ingresoMensual*14 + bono + utilidades
            ea.SM + ea.EC_Coms                  AS EC_IngresoMensual,
            (ea.SM + ea.EC_Coms) * 14.0
            + ea.EC_BonoCPTarget + ea.EC_Utilidades  AS EC_IngresoAnual
        FROM CalcECAnual ea
    ),
    ...
    Union_Resultados AS (
        ...
        SELECT  -- Colombia
            ...
            -- Nuevos (2026-08-19)
            co.CO_IngresoMensual, co.CO_IngresoAnual,
            NULL AS PE_IngresoMensual, NULL AS PE_IngresoAnual,
            NULL AS EC_IngresoMensual, NULL AS EC_IngresoAnual,
            -- PE (NULL)
        ...
        SELECT  -- Perú
            ...
            -- Nuevos (2026-08-19)
            NULL AS CO_IngresoMensual, NULL AS CO_IngresoAnual,
            pa.PE_IngresoMensual, pa.PE_IngresoAnual,
            NULL AS EC_IngresoMensual, NULL AS EC_IngresoAnual,
            -- PE
        ...
        SELECT  -- Ecuador
            ...
            NULL AS CO_IngresoMensual, NULL AS CO_IngresoAnual,
            NULL AS PE_IngresoMensual, NULL AS PE_IngresoAnual,
            ec.EC_IngresoMensual, ec.EC_IngresoAnual,
            -- PE (NULL)
    ...
    WHEN MATCHED THEN UPDATE SET
        ...
        -- Nuevos (2026-08-19)
        T.CO_IngresoMensual         = S.CO_IngresoMensual,
        T.CO_IngresoAnual           = S.CO_IngresoAnual,
        T.PE_IngresoMensual         = S.PE_IngresoMensual,
        T.PE_IngresoAnual           = S.PE_IngresoAnual,
        T.EC_IngresoMensual         = S.EC_IngresoMensual,
        T.EC_IngresoAnual           = S.EC_IngresoAnual,
    ...
    WHEN NOT MATCHED THEN INSERT (
        ...
        CO_IngresoMensual, CO_IngresoAnual, PE_IngresoMensual, PE_IngresoAnual, EC_IngresoMensual, EC_IngresoAnual,
        FechaCalculo
    ) VALUES (
        ...
        S.CO_IngresoMensual, S.CO_IngresoAnual, S.PE_IngresoMensual, S.PE_IngresoAnual, S.EC_IngresoMensual, S.EC_IngresoAnual,
        GETDATE()
    );

    -- Capturar el rowcount INMEDIATAMENTE después del MERGE en una variable local.
    -- En Azure SQL, si la conexión se recupera durante el batch, @@ROWCOUNT queda
    -- inválido (error 4083); la variable conserva el valor real.
    SET @FilasProcesadas = @@ROWCOUNT;

    PRINT 'sp_CalcularCostos OK — ' + CAST(@FilasProcesadas AS VARCHAR) + ' filas procesadas.';
END;
GO